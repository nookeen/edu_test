...
"use strict";
  ...
  function renderTestPages(){
    ...
    for (var pageClass in pages)
      _createPageObject(pages[pageClass]);
    
    if(data.length > 1){
      _(data).reverse();
      
      for (var pageClass in pages)
        _buildTemplate(pages[pageClass]);
    }
  }
  function _createPageObject(pageClass) {
    ...
    //console.p(pageObject);
    //console.p(pageClass);
    
    if(pageClass === '.singleQuestion'){
  ...
  function _buildTemplate(pageClass) {
    ...
    for (var key in data) {
      ...
      for (var key2 in data[key]) {
        if(pageClass === '.questions') {
          
          console.p(data[key][key2]);
          
          //testsPage.template = {
          //  content: data[key][key2].questionTitle,
          //  uniqueId: data[key][key2].questionUniqueID,
          //  jHtml: testsPage.jLi.clone()
          //                      .attr('data-unique-id', data[key][key2].questionUniqueID)
          //                      .html(data[key][[key2].questionTitle);
          //}
          
          for (var key3 in data[key2]) {
          if(pageClass === '.answer') {
            
            //console.p(data[key][key2][key3]);
            //console.p(data[key2][key3]);
            console.p(data[key]);
